...
trait EventOps:
  extension (event: Event)
    ...
    def isStrictlyLess(other: Event): Boolean =
      isLessOrEqual(other) && !other.isLessOrEqual(event)

    def isLessOrEqual(other: Event): Boolean =
      Leq(event, other)

    def isConcurrent(other: Event): Boolean =
      !event.isLessOrEqual(other) && !other.isLessOrEqual(event)

    def isEqual(other: Event): Boolean =
      event.isLessOrEqual(other) && other.isLessOrEqual(event)
  end extension
