...
class EventSpec extends ScalaCheckSuite:
  ...
  test(
    ...
    Prop.forAll(eventGen, identityGen) { (event, identity) =>
      assert(event.isLessOrEqual(event.fill(identity)))
    }
  }
  ...
  test(
    ...
    Prop.forAll(eventGen, identityGen.suchThat(_ != itcId"0")) {
      (event, identity) =>
        ...
        assert(event.isStrictlyLess(grown))
    }
  }
  ...
  test(
    ...
    Prop.forAll(eventGen, identityGen.suchThat(_ != itcId"0")) {
      (event, identity) =>
        ...
        assert(grown1.isConcurrent(grown2))
    }
  }

  test("isConcurrent commutes") {
    Prop.forAll(eventGen, eventGen)((ev1, ev2) =>
      assertEquals(ev1.isConcurrent(ev2), ev2.isConcurrent(ev1)),
    )
  }
